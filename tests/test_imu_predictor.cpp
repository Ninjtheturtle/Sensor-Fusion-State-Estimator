...
TEST(ImuPredictor, CovarianceGrowsWithTime) {
    ...
    // using local vars — comma-in-template confuses EXPECT_GT macro
...
TEST(ImuPredictor, ConstantAccelGivesQuadraticPosition) {
    // set sensor reads [0.5, 0, 9.81]: after gravity comp, corrected = [0.5, 0, 0] m/s^2
    ESKFConfig cfg;
    ImuPredictor pred(cfg);
